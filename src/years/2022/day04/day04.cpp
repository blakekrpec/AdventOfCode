...
std::pair<Range, Range> parseLineIntoElfRanges(const std::string &line)
{
	...
	Range elf1_range = {std::stoi(line.substr(0, hyphen1)), std::stoi(line.substr(hyphen1 + 1, comma - hyphen1 - 1))};

	Range elf2_range = {std::stoi(line.substr(comma + 1, hyphen2 - comma - 1)), std::stoi(line.substr(hyphen2 + 1))};
...
int main()
{
	const auto lines = aoc::readLines("src/years/2022/day04/input/input.txt");
